...
namespace DeltaQ.RTB.Agent
{
	public class BackupAgent : DiagnosticOutputBase, IBackupAgent
	{
			...
			switch (action)
			{
				...
				default:
					...
					_errorLogger.LogError(
						...
						"DeltaQ.RTB: Backup Queue Internal Error");
		...
		void UploadThreadProc(int threadIndex, CancellationToken cancellationToken)
		{
			...
			{
				// This shouldn't ever happen.
				_errorLogger.LogError("Was started without the _uploadThreadStatus array initialized", "DeltaQ.RTB: Upload Thread Internal Error");
