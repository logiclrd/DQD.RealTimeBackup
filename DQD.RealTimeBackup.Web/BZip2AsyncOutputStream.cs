...
namespace ICSharpCode.SharpZipLib.BZip2
{
	...
	/// </summary>
	public class BZip2AsyncOutputStream : Stream
	{
		#region Constants
		...
		*/
		...
		private BZip2AsyncOutputStream(Stream baseStream)
		{
			this.baseStream = baseStream;

			block = Array.Empty<byte>();
			quadrant = Array.Empty<int>();
			zptr = Array.Empty<int>();
			szptr = Array.Empty<short>();
			ftab = Array.Empty<int>();
		}
		...
		public static Task<BZip2AsyncOutputStream> CreateAsync(Stream stream)
		...
		public static Task<BZip2AsyncOutputStream> CreateAsync(Stream stream, CancellationToken cancellationToken)
			=> CreateAsync(stream, 9, cancellationToken);

		/// <summary>
		/// Initialise a new instance of the <see cref="BZip2AsyncOutputStream"></see>
		...
		/// </remarks>
		public static Task<BZip2AsyncOutputStream> CreateAsync(Stream stream, int blockSize)
			=> CreateAsync(stream, blockSize, CancellationToken.None);
		...
		/// the lowest compression and 9 the highest.
		/// </remarks>
		public static async Task<BZip2AsyncOutputStream> CreateAsync(Stream stream, int blockSize, CancellationToken cancellationToken)
		{
			if (stream == null)
				throw new ArgumentNullException(nameof(stream));

			BZip2AsyncOutputStream @this = new BZip2AsyncOutputStream(stream);

			await @this.Construct(blockSize, cancellationToken);

			return @this;
		}

		async Task Construct(int blockSize, CancellationToken cancellationToken)
		{
			bsLive = 0;
			bsBuff = 0;
			bytesOut = 0;
		...
		/// are performed when the garbage collector reclaims the BZip2AsyncOutputStream.
		/// </summary>
		...
		public override long Position
		{
			...
			set
			{
				throw new NotSupportedException("BZip2AsyncOutputStream position cannot be set");
			}
		}
		...
		{
			throw new NotSupportedException("BZip2AsyncOutputStream Seek not supported");
		}
		...
		{
			throw new NotSupportedException("BZip2AsyncOutputStream SetLength not supported");
		}
		...
		{
			throw new NotSupportedException("BZip2AsyncOutputStream ReadByte not supported");
		}
		...
		{
			throw new NotSupportedException("BZip2AsyncOutputStream Read not supported");
		}
		...
		{
			Wait(WriteAsync(buffer, offset, count, CancellationToken.None));
		}

		/// <summary>
		/// Write a block of bytes to the stream asynchronously
		/// </summary>
		/// <param name="buffer">The buffer containing data to write.</param>
		/// <param name="offset">The offset of the first byte to write.</param>
		/// <param name="count">The number of bytes to write.</param>
		...
		public override async Task WriteAsync(byte[] buffer, int offset, int count, CancellationToken cancellationToken)
		{
			...
			{
				await WriteByteAsync(buffer[offset + i], cancellationToken);
			}
		}
		...
		{
			throw new NotSupportedException("BZip2AsyncOutputStream synchronous WriteByte not supported");
		}
			...
			try
			{
				...
				if (currentChar != -1)
				{
					if (currentChar == b)
					{
						...
						if (runLength > 254)
						{
							await WriteRunAsync(cancellationToken);
							currentChar = -1;
							runLength = 0;
						}
					}
		...
		/// </summary>
		private async Task WriteRunAsync(CancellationToken cancellationToken)
		{
			...
			{
				await EndBlockAsync(cancellationToken);
				InitBlock();
				await WriteRunAsync(cancellationToken);
			}
		}
		...
		/// <summary>
		/// Releases the unmanaged resources used by the <see cref="BZip2AsyncOutputStream"/> and optionally releases the managed resources.
		/// </summary>
			...
			try
			{
				try
				{
					...
					if (!disposed_)
					{
						...
						if (runLength > 0)
						{
							Wait(WriteRunAsync(CancellationToken.None));
						}

						currentChar = -1;
						Wait(EndBlockAsync(CancellationToken.None));
						Wait(EndCompressionAsync(CancellationToken.None));
						Flush();
					}
				}
				finally
		...
		private async Task InitializeAsync()
		{
			...
			await BsPutUCharAsync('B');
			await BsPutUCharAsync('Z');

			await BsPutUCharAsync('h');
			await BsPutUCharAsync('0' + blockSize100k);
		...
		private async Task EndBlockAsync(CancellationToken cancellationToken)
		{
			if (last < 0)
			...
			damaged files.
			--*/
			await BsPutUCharAsync(0x31);
			await BsPutUCharAsync(0x41);
			await BsPutUCharAsync(0x59);
			await BsPutUCharAsync(0x26);
			await BsPutUCharAsync(0x53);
			await BsPutUCharAsync(0x59);

			/*-- Now the block's CRC, so it is in a known place. --*/
			unchecked
			{
				await BsPutintAsync((int)blockCRC);
			}
			...
			await MoveToFrontCodeAndSendAsync(cancellationToken);
		}

		private async Task EndCompressionAsync(CancellationToken cancellationToken)
		{
			_cancellationToken = cancellationToken;
			...
			--*/
			await BsPutUCharAsync(0x17);
			await BsPutUCharAsync(0x72);
			await BsPutUCharAsync(0x45);
			await BsPutUCharAsync(0x38);
			await BsPutUCharAsync(0x50);
			await BsPutUCharAsync(0x90);

			unchecked
			{
				await BsPutintAsync((int)combinedCRC);
			}

			await BsFinishedWithStreamAsync();
		}

		private async Task BsFinishedWithStreamAsync()
		{
			while (bsLive > 0)
			{
				int ch = (bsBuff >> 24);
				await EmitByte((byte)ch, _cancellationToken); // write 8-bit
				bsBuff <<= 8;
				bsLive -= 8;
				bytesOut++;
			}

			await FlushEmitByteBufferAsync(_cancellationToken);
		}

		byte[] _writeBuffer = new byte[16384];
		int _writeBufferOffset = 0;

		async Task EmitByte(byte byteValue, CancellationToken cancellationToken)
		{
			_writeBuffer[_writeBufferOffset++] = byteValue;

			if (_writeBufferOffset == _writeBuffer.Length)
				await FlushEmitByteBufferAsync(cancellationToken);
		}

		async Task FlushEmitByteBufferAsync(CancellationToken cancellationToken)
		{
			await baseStream.WriteAsync(_writeBuffer, 0, _writeBufferOffset, _cancellationToken);
			_writeBufferOffset = 0;
		}

		private async Task BsWAsync(int n, int v)
		{
			while (bsLive >= 8)
			{
				int ch = (bsBuff >> 24);
				await EmitByte(unchecked((byte)ch), _cancellationToken); // write 8-bit
				bsBuff <<= 8;
				bsLive -= 8;
				++bytesOut;
			}
		...
		private Task BsPutUCharAsync(int c)
		{
			return BsWAsync(8, c);
		}

		private async Task BsPutintAsync(int u)
		{
			await BsWAsync(8, (u >> 24) & 0xFF);
			await BsWAsync(8, (u >> 16) & 0xFF);
			await BsWAsync(8, (u >> 8) & 0xFF);
			await BsWAsync(8, u & 0xFF);
		}

		private Task BsPutIntVSAsync(int numBits, int c)
		{
			return BsWAsync(numBits, c);
		}

		private async Task SendMTFValuesAsync()
		{
			...
			/*--- Now the selectors. ---*/
			await BsWAsync(3, nGroups);
			await BsWAsync(15, nSelectors);
			for (int i = 0; i < nSelectors; ++i)
			{
			...
			while (true)
			{
				...
				for (int i = gs; i <= ge; i++)
				{
					await BsWAsync(len[selector[selCtr]][szptr[i]], code[selector[selCtr]][szptr[i]]);
				}
		...
		private async Task MoveToFrontCodeAndSendAsync(CancellationToken cancellationToken)
		{
			_cancellationToken = cancellationToken;

			try
			{
				await BsPutIntVSAsync(24, origPtr);
				GenerateMTFValues();
				await SendMTFValuesAsync();
			}
			finally
			{
				_cancellationToken = CancellationToken.None;
