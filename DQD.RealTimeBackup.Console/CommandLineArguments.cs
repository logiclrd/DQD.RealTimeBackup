...
namespace DQD.RealTimeBackup.Console
{
	public class CommandLineArguments
	{
		[Argument("/CONNECT", Description =
			"The endpoint to which to connect. Can be a UNIX socket (e.g. /run/DQD.RealTimeBackup/bridge.socket) " +
			"or a TCP/IP endpoint (e.g. 127.0.0.1:12345 -- check /run/DQD.RealTimeBackup/bridge-tcp-port for the " +
		...
		[Switch("/PERFORMRESCAN", Description =
			"Instructs the DQD.RealTimeBackup service to begin a rescan. This scan is exactly the same " +
		...
		[Switch("/FOLLOWRESCAN", Description =
			...
			"statistics about the scan. When the scan completes, DQD.RealTimeBackup.Console exits. This " +
		...
		[Switch("/CANCELRESCAN", Description =
			"Instructs the DQD.RealTimeBackup to stop any ongoing rescan, whether it was started by the " +
